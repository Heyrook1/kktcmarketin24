...
export async function fetchCurrencyRates(force: boolean) {
  const now = Date.now()
  if (!force && memoryCache && now < memoryCache.expiresAt) {
    return { payload: memoryCache.payload, cached: true }
  }
  ...
  try {
    ...
    if (!normalized) throw new Error("Invalid currency payload")
    ...
    return { payload, cached: false, fallback: true }
  }
}
