...
const threadMessageSchema = z.object({
  message: z
    ...
    .max(2000, "Mesaj 2000 karakteri gecemez."),
})
...
export async function POST(
  ...
  try {
    ...
    const parsedPayload = threadMessageSchema.safeParse(payload)
    if (!parsedPayload.success) {
      const message = parsedPayload.error.issues[0]?.message ?? "Gecersiz mesaj."
