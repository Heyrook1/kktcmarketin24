...
export async function POST(req: NextRequest) {
  try {
    ...
    // If Resend is not configured, skip silently (fire-and-forget from client)
    ...
    const customerHtml = `
      <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;color:#111">
        ...
        <div style="background:#fff;padding:24px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
          ...
          <p style="color:#6b7280;font-size:12px;margin-top:24px">Sorularınız için: <a href="mailto:info@marketin24.com" style="color:#2563eb">info@marketin24.com</a></p>
        </div>
      </div>`
    ...
    // Vendor emails — fetch vendor user emails via vendor_stores → profiles join
      ...
      if (ownerIds.length > 0) {
        ...
        for (const profile of (profiles ?? []) as Array<{ id: string; email: string | null; full_name: string | null }>) {
          ...
          const vendorStore = (vendorStores ?? []).find(
            (s: { owner_id: string }) => s.owner_id === profile.id
          )
          const vendorHtml = `
            <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;color:#111">
              <div style="background:#0f172a;padding:24px 32px;border-radius:12px 12px 0 0">
              ...
              <div style="background:#fff;padding:24px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
                ...
                <p style="color:#6b7280;font-size:12px;margin-top:24px">Sipariş paneli: <a href="https://marketin24.com/vendor-panel" style="color:#2563eb">vendor-panel</a></p>
              </div>
            </div>`
