...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const parsed = updateVendorOrderStatusSchema.safeParse(await req.json().catch(() => null))
    if (!parsed.success) {
      return NextResponse.json({ error: parsed.error.issues[0]?.message ?? 'Geçersiz JSON gövdesi.' }, { status: 400 })
    }
