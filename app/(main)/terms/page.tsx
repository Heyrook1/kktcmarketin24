...
const SECTIONS = [
  ...
  },
  {
    ...
    content:
      "Platform logosu, tasarımı, kodu ve içeriği Marketin24'e aittir. Satıcılar, kendi ürünlerine ilişkin içeriklerin haklarını elinde bulundurmakta ve bu içerikleri yayımlamak için gerekli lisanslara sahip olduklarını beyan etmektedir. İçerik ihlali iddialarını info@marketin24.com adresine bildirebilirsiniz.",
  },
  {
    id: "sorumluluk",
    title: "9. Sorumluluk Sınırlaması",
    content:
      "Platform, satıcıların ürün kalitesi ve teslimat performansı dahil olmak üzere kullanıcıların uğrayabileceği dolaylı veya özel zararlardan sorumlu değildir. Doğrudan zararlar bakımından azami sorumluluk tutarı, ilgili işlem bedelini aşamaz.",
  },
  {
    id: "hukuk",
    title: "10. Uygulanacak Hukuk",
    content:
      "Bu şartlar, Kuzey Kıbrıs Türk Cumhuriyeti hukukuna tabidir. Uyuşmazlıklar öncelikle taraflar arasında müzakere yoluyla çözülmeye çalışılır; çözüme kavuşturulamazsa Lefkoşa mahkemelerinde görülür.",
  },
  {
    id: "iletisim",
    title: "11. İletişim",
    content:
      "Bu şartlar hakkındaki sorularınız için: info@marketin24.com | +90 533 873 43 17 | Lefkoşa, KKTC",
  },
]
...
export default function TermsPage() {
  return (
    <div className="min-h-screen bg-background font-sans">
      <header className="bg-[oklch(0.22_0.06_255)] px-4 pt-6 pb-12 text-white md:pb-16">
        <div className="container mx-auto max-w-4xl">
          ...
          <p className="mt-2 text-sm text-primary-foreground/75">Son güncelleme: {LAST_UPDATED}</p>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {SECTIONS.map((section) => (
              <a
                key={section.id}
                href={`#${section.id}`}
                className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs text-white/80 transition-colors hover:bg-white/20 hover:text-white"
              >
                {section.title}
              </a>
            ))}
        ...
        <div className="grid gap-10 lg:grid-cols-[220px_1fr]">
          <nav aria-label="İçindekiler" className="hidden lg:block">
            <div className="sticky top-20 space-y-1 rounded-xl border bg-card p-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">İçindekiler</p>
              {SECTIONS.map((section) => (
                <a
                  key={section.id}
                  href={`#${section.id}`}
          ...
          <article className="space-y-10 lg:col-span-3">
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm leading-relaxed text-primary">
              Bu şartlar platformumuzu kullanmadan önce dikkatlice okunmalıdır. Platformu kullanmaya devam ederek bu
