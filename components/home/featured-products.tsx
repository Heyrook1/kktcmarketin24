...
function toProduct(p: {
  ...
  created_at: string
  store_id: string
  vendor_stores?: { name: string | null; slug: string | null } | { name: string | null; slug: string | null }[] | null
}): Product {
...
async function fetchProducts(opts: {
  ...
  let query = supabase
    ...
    .select("id, name, description, price, compare_price, category, image_url, images, tags, stock, created_at, store_id, vendor_stores(name, slug)")
    .eq("is_active", true)
    .gt("stock", 0)
    .limit(targetCount * 3)
  ...
  return (data ?? [])
    .filter((p) => isPublicInStockProduct(p))
    .slice(0, targetCount)
